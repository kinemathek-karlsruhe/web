...
<article class="mx-auto max-w-3xl p-6">
  ...
  <dl class="grid grid-cols-[max-content_1fr] gap-x-4">
    <?php if ($page->originalTitle()->isNotEmpty()): ?><dt class="font-semibold">Originaltitel</dt><dd><?= html($page->originalTitle()) ?></dd><?php endif ?>
    <?php if ($page->year()->isNotEmpty()): ?><dt class="font-semibold">Jahr</dt><dd><?= html($page->year()) ?></dd><?php endif ?>
    <?php if ($page->country()->isNotEmpty()): ?><dt class="font-semibold">Land</dt><dd><?= html($page->country()->commaList()) ?></dd><?php endif ?>
    <?php if ($page->language()->isNotEmpty()): ?><dt class="font-semibold">Sprache</dt><dd><?= html($page->language()->commaList()) ?></dd><?php endif ?>
    <?php if ($page->runtime()->isNotEmpty()): ?><dt class="font-semibold">Laufzeit</dt><dd><?= html($page->runtime()) ?> min</dd><?php endif ?>
    <?php if ($page->genre()->isNotEmpty()): ?><dt class="font-semibold">Genre</dt><dd><?= html($page->genre()->commaList()) ?></dd><?php endif ?>
  </dl>

  <?php if ($directors->count() > 0): ?>
    <h2 class="font-semibold mt-4">Regie</h2>
    <ul><?php foreach ($directors as $director): ?><li><?= html($director->name()) ?></li><?php endforeach ?></ul>
  <?php endif ?>

  <?php if ($page->synopsis()->isNotEmpty()): ?>
    <h2 class="font-semibold mt-4">Inhalt</h2>
    <div class="prose"><?= $page->synopsis()->kt() ?></div>
  <?php endif ?>

  <?php $stills = $page->stills()->toFiles() ?>
  <?php if ($stills->count() > 0): ?>
    <h2 class="font-semibold mt-4">Szenenbilder</h2>
  ...
  <h2 class="font-semibold mt-6">Kommende Vorstellungen</h2>
  ...
  <?php else: ?>
    <ul>
      <?php foreach ($upcoming as $showing): ?>
        <li class="py-1">
          <a class="underline" href="<?= $showing->url() ?>"><?= html($showing->date()->toDate('D, d.m.Y H:i')) ?> Uhr</a>
          <?php if ($showing->subtitles()->isNotEmpty()): ?> · <?= html($showing->subtitles()->commaList()) ?><?php endif ?>
          <?php if ($showing->ticketUrl()->isNotEmpty()): ?> · <a href="<?= $showing->ticketUrl()->esc() ?>" rel="noopener noreferrer">Tickets</a><?php endif ?>
  ...
  <h2 class="font-semibold mt-6">Frühere Vorstellungen</h2>
  <?php if ($past->count() === 0): ?>
  ...
  <?php else: ?>
    <ul class="text-gray-500">
      <?php foreach ($past as $showing): ?>
        ...
        <li class="py-1"><?= html($showing->date()->toDate('D, d.m.Y H:i')) ?> Uhr<?php if ($showing->subtitles()->isNotEmpty()): ?> · <?= html($showing->subtitles()->commaList()) ?><?php endif ?></li>
      <?php endforeach ?>
    </ul>
  <?php endif ?>
